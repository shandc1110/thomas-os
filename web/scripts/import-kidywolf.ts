/**
 * Import Kidywolf catalogue from littleCONCEPTS Shopify collection (UK GBP RRP).
 * Source: https://www.littleconcepts.co.uk/collections/kidywolf
 *
 *   npx tsx scripts/import-kidywolf.ts
 *   npx tsx scripts/import-kidywolf.ts --dry
 */
...
const BRAND = "Kidywolf";
const BASE = "https://www.littleconcepts.co.uk";
const COLLECTION = "kidywolf";
const PRESell_MONTH = "2026-10";
const DEFAULT_PRESell_QTY = 50;
const dryRun = process.argv.includes("--dry");

function cleanTitle(title: string): string {
  return title
    .replace(/\s+by\s+Kidywolf\s*$/i, "")
    .replace(/\s+/g, " ")
    .trim();
}

function listingSku(handle: string): string {
  return `KW-LIST-${handle}`.slice(0, 64);
}
...
function buildTags(
  ...
  const tags = [
    ...shopifyTags,
    "source:littleconcepts",
    `cbc_vgroup:${handle}`,
    role === "listing" ? "cbc_listing" : "cbc_variant",
  ];
...
async function main() {
  ...
  console.log(`Fetching Kidywolf collection from ${BASE}/collections/${COLLECTION}…`);
  const products = await fetchShopifyCollectionProducts(BASE, COLLECTION);
  const sellable = products.filter((p) => {
    const vendor = String(p.vendor ?? "").trim().toLowerCase();
    if (vendor && vendor !== "kidywolf") return false;
  ...
  console.log(`Products (Kidywolf, excl. bundles): ${sellable.length}`);
  ...
  for (const raw of sellable) {
    const title = cleanTitle(String(raw.title ?? "").trim());
    ...
    const images = Array.isArray(raw.images)
      ...
      : [];
    const gallery = images.map((img) => img.src).filter((src): src is string => Boolean(src));
    const defaultImage = gallery[0] ?? "";
    ...
    if (dryRun) {
      console.log(
        `  [dry] ${handle} → ${variantCount} SKU(s), from £${minPrice.toFixed(2)} (${title})`,
      );
      parents++;
    ...
    if (variantCount === 1) {
      ...
      const galleryRest = gallery.filter((url) => url !== (v.imageUrl || defaultImage));
      const payload = {
        ...basePayload,
        sku: v.sku,
        name: title,
        ...
        image_url: v.imageUrl || defaultImage || null,
  ...
  else console.log("Storefront: /brands/kidywolf");
}
